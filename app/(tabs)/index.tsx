...
export default function Home() {
  ...
  const completed = monthVisits.filter(
    (visit) => visit.status === 'completed'
  ).length;

  const planned = monthVisits.filter(
  ...
  return (
    ...
    >
      ...
      <View style={[styles.hero, { backgroundColor: c.navy }]}>
        <View style={styles.heroText}>
          ...
          <Text style={styles.heroTitle}>
            سجّل زياراتك بوضوح، وابقَ قريباً من كل مدرسة.
          </Text>

          <Text style={styles.heroSub}>
            إدارة السجلات والخطط والكتب الوزارية من مكان واحد
          </Text>
        </View>
      ...
      <View style={styles.metrics}>
        <Metric icon="home" label="المدارس" value={schools.length} c={c} />
        <Metric icon="check-circle" label="زيارات الشهر" value={completed} c={c} />
        <Metric icon="clock" label="مخططة" value={planned} c={c} />
      </View>
      ...
      <View style={styles.quickGrid}>
        ...
        <Quick
          ...
          onPress={() => router.push('/ministry-books')}
          c={c}
        />
      </View>
      ...
      {recentVisits.length === 0 ? (
        ...
        >
          ...
          <Text style={[styles.emptyVisitsTitle, { color: c.foreground }]}>
            لا توجد زيارات هذا الشهر
          </Text>

          <Text style={[styles.emptyVisitsText, { color: c.mutedForeground }]}>
            يمكنك تسجيل زيارة جديدة من الإجراءات السريعة.
          </Text>
        </View>
      ) : (
        recentVisits.map((visit) => (
          ...
          >
            <View style={[styles.dateBox, { backgroundColor: c.secondary }]}>
              ...
              <Text style={[styles.dateMonth, { color: c.mutedForeground }]}>
                {getArabicMonth(visit.date)}
              </Text>
            </View>
      ...
      <View style={[styles.todayCard, { backgroundColor: c.secondary }]}>
        ...
        <View style={styles.todayText}>
          <Text style={[styles.todayTitle, { color: c.foreground }]}>
            أعمال اليوم
          </Text>
          <Text style={[styles.todaySub, { color: c.mutedForeground }]}>
            لديك {todayTasks.length} {todayTasks.length === 1 ? 'مهمة' : 'مهام'} مسجلة لليوم.
          </Text>
        </View>
      ...
      <View style={[styles.offline, { backgroundColor: c.secondary }]}>
        ...
        >
          تعمل دون إنترنت · ستتم مزامنة البيانات عند توفر الاتصال
        </Text>
      </View>

      <Text style={[styles.credit, { color: c.mutedForeground }]}>
        تصميم وبرمجة: الاختصاص التربوي ضرغام مهدي
      </Text>
    </ScrollView>
  );
}
...
function Metric({ icon, label, value, c }: any) {
  return (
    <View style={[styles.metric, { backgroundColor: c.card, borderColor: c.border }]}>
