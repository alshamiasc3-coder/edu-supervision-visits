...
export default function Home() {
  ...
  return (
    ...
    >
      {/* رأس الصفحة: بدون اسم شخصي حتى يناسب تعدد المستخدمين */}
      <View style={styles.top}>
        <View style={styles.topText}>
          <Text style={[styles.eyebrow, { color: c.primary }]}>نظام الإشراف التربوي</Text>
          <Text style={[styles.greeting, { color: c.foreground }]}>المتابعة اليومية والإشراف التربوي</Text>
        </View>
      ...
      {/* الإحصائيات أصبحت أزرار وصول سريع */}
      <View style={styles.metrics}>
        <Metric icon="home" label="المدارس" value={schools.length} c={c} onPress={() => router.push('/(tabs)/schools')} />
        <Metric icon="check-circle" label="زيارات مكتملة" value={completed} c={c} onPress={() => router.push('/(tabs)/visits')} />
        <Metric icon="clock" label="زيارات مخططة" value={planned} c={c} onPress={() => router.push('/(tabs)/visits')} />
      </View>
      ...
      <View style={styles.quickGrid}>
        ...
        <Quick icon="check-square" label="أعمال اليوم" onPress={() => router.push('/(tabs)/tasks')} c={c} />
      ...
      <View style={[styles.offline, { backgroundColor: c.secondary }]}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={[styles.offlineText, { color: c.secondaryForeground }]}>يعمل التطبيق دون إنترنت · ستتم مزامنة البيانات عند توفر الاتصال</Text>
          <Text style={[styles.aiNote, { color: c.mutedForeground }]}>المساعد الذكي يحتاج إلى اتصال بالإنترنت</Text>
        </View>
      </View>
...
function Metric({ icon, label, value, c, onPress }: any) {
  return (
    <Pressable
      onPress={onPress}
      style={({ pressed }) => [styles.metric, { backgroundColor: c.card, borderColor: c.border, opacity: pressed ? 0.75 : 1 }]}
    >
...
function Status({ status, c }: any) {
  const backgroundColor = status === 'completed' ? c.muted : status === 'postponed' ? '#FFF2DD' : c.secondary;
...
const styles = StyleSheet.create({
  ...
  heroText: { flex: 1 },
  heroKicker: { color: '#C6DDE4', fontSize: 12, fontFamily: 'Inter_600SemiBold', textAlign: 'right', marginBottom: 9 },
  heroTitle: { color: '#FFFFFF', fontSize: 18, lineHeight: 27, fontFamily: 'Inter_700Bold', textAlign: 'right' },
  heroSub: { color: '#C6DDE4', fontSize: 11, lineHeight: 17, fontFamily: 'Inter_400Regular', textAlign: 'right', marginTop: 9 },
  ...
  metric: { flex: 1, borderRadius: 16, borderWidth: 1, padding: 11, alignItems: 'center', minHeight: 84 },
  metricValue: { fontSize: 20, fontFamily: 'Inter_700Bold', marginTop: 4 },
  metricLabel: { fontSize: 10, fontFamily: 'Inter_500Medium', marginTop: 3, textAlign: 'center' },
  ...
  offlineText: { fontFamily: 'Inter_500Medium', fontSize: 11, textAlign: 'right' },
  aiNote: { fontFamily: 'Inter_400Regular', fontSize: 9, textAlign: 'right', marginTop: 3 },
