...
export default function MinistryBooks() {
  ...
  const saveBook = () => {
    ...
    if (!bookNumber.trim()) { Alert.alert('تنبيه', 'اكتب رقم الكتاب الوزاري.'); return; }
  ...
  const openPdf = async (uri: string) => {
    ...
    catch { Alert.alert('تعذر فتح الملف', 'لا يمكن فتح ملف PDF على هذا الجهاز حالياً.'); }
  };
    ...
    <View style={[styles.container, { backgroundColor: c.background }]}>
      ...
      <Modal visible={!!preview} animationType="fade" transparent onRequestClose={() => setPreview(null)}>
        <View style={styles.previewOverlay}>
          ...
          {preview?.type === 'image' ? <>
            <View style={styles.previewContainer}><ScrollView horizontal contentContainerStyle={styles.zoomScroll} maximumZoomScale={4} minimumZoomScale={1} showsHorizontalScrollIndicator={false} showsVerticalScrollIndicator={false} centerContent><Image source={{ uri: preview.uri }} style={[styles.fullPreviewImage, { transform: [{ scale: zoom }] }]} resizeMode="contain" /></ScrollView></View>
            <View style={styles.zoomControls}>
              <Pressable onPress={() => setZoom(z => Math.min(4, Number((z + 0.25).toFixed(2))))} style={styles.zoomButton}><Feather name="plus" size={22} color="#FFFFFF" /></Pressable>
              <Text style={styles.zoomText}>{Math.round(zoom * 100)}%</Text>
              <Pressable onPress={() => setZoom(z => Math.max(1, Number((z - 0.25).toFixed(2))))} style={styles.zoomButton}><Feather name="minus" size={22} color="#FFFFFF" /></Pressable>
              <Pressable onPress={() => setZoom(1)} style={styles.zoomReset}><Text style={styles.previewBottomText}>إعادة</Text></Pressable>
            </View>
...
function BookCard({ book, c, categoryBackground, onAttachmentPress, onPdfPress, onEdit, onDelete }: any) {
  return <View style={[styles.bookCard, { backgroundColor: c.card, borderColor: c.border }]}>
    ...
    <View style={styles.bookInfo}><View style={styles.bookTopRow}><View style={[styles.categoryBadge, { backgroundColor: categoryBackground }]}><Text style={[styles.categoryText, { color: c.primary }]}>{book.category}</Text></View><Text style={[styles.bookTitle, { color: c.foreground }]} numberOfLines={2}>{book.title}</Text></View>
      <Text style={[styles.bookNumber, { color: c.mutedForeground }]}>رقم الكتاب: {book.bookNumber}</Text><Text style={[styles.bookDate, { color: c.mutedForeground }]}>تاريخ الكتاب: {book.date}</Text>
